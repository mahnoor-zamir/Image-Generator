    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setImages(responseData.images); // Set images state with URLs received from backend
            } else {
                console.error('Failed to generate images:', response.statusText);
            }
        } catch (error) {
    ...
    return (
        <main>
            <div className={"max-w-5xl mx-auto px-5 lg:px-0"}>
                <h1 className={"inline-block text-transparent bg-clip-text py-4 text-6xl font-bold bg-gradient-to-r from-[#009FFF] to-[#ec2F4B] font-squarePeg"}>
                    ImaginAI
                </h1>
            </div>
                ...
                <div className={"mt-4 pt-4 border-t"}>
                    {loading ? (
                        ...
                        <div className={"grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4"}>
                            {images.map((imageUrl, index) => <ImageCard key={index} url={imageUrl} />)}
                        </div>
                    ) : (
                        <div className={"h-100 flex flex-col justify-center items-center"}>
